...
const KnightModel: React.FC<KnightModelProps> = ({ scene, environmentMap }) => {
  ...
  useEffect(() => {
    if (!scene) {
      console.error("KnightModel: No scene provided");
      return;
    }

    try {
      console.log("KnightModel: Creating knight model");
      ...
      const createMaterial = (color = 0xf0f0f0, metalness = 0.7, roughness = 0.3) => {
        const material = new THREE.MeshPhysicalMaterial({
          ...
          metalness,
          roughness,
          envMap: environmentMap || undefined,
          envMapIntensity: environmentMap ? 1.0 : 0.0,
          clearcoat: 0.3,
      ...
      console.log("KnightModel: Knight model created successfully");

    } catch (error) {
      console.error("KnightModel: Failed to create knight model", error);
    }

    // Clean up on unmount
    return () => {
      if (knightGroupRef.current && scene) {
        console.log("KnightModel: Cleaning up knight model");
        scene.remove(knightGroupRef.current);
        
        // Dispose of geometries and materials to free memory
        knightGroupRef.current.traverse((object) => {
