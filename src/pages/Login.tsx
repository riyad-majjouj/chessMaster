...
import logAnimation from "../../public/logAnimation.json";
import { toast } from "@/hooks/use-toast";
import { Container } from "@/components/ui/container";
import Navbar from "@/components/Navbar";
import { useAuth } from "@/context/AuthContext"; // <-- استيراد الهوك
...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && responseData.session?.access_token && responseData.user) {
        // استدعاء دالة login من السياق لتحديث الحالة في كل التطبيق
        login(responseData.session.access_token, responseData.user);
        toast({
          title: "Success",
          description: "You have successfully logged in!",
        });
        // التوجيه سيتم تلقائياً من داخل دالة login
      } else {
        toast({
          title: "Error",
                ...
                <div className="glass-card p-8 rounded-xl w-full max-w-md">
                  ...
                  <form onSubmit={handleLogin} className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="bg-white/10 border-white/20 text-white" placeholder="••••••••" required />
                    </div>
                  ...
                  <div className="mt-6 text-center">
                    <p className="text-sm text-muted-foreground">Don't have an account? <Link to="/signup" className="text-gold hover:underline">Sign up</Link></p>
                  </div>
                </div>
              </div>
              <div className="hidden lg:block h-[500px]">
                <Lottie animationData={logAnimation} loop={true} autoplay={true} style={{ width: 'auto', height: 'auto', overflow: 'hidden' }} />
              </div>
            </div>
          </Container>
        </div>
